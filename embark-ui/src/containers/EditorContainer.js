...
class EditorContainer extends React.Component {
  ...
  openAsideTab(newTab) {
    if (!this.state.isSmallSize) {
      this.changeEditorWidth((this.state.currentAsideTab === '') ? OPERATIONS.LESS : OPERATIONS.MORE);
    }
  ...
  renderTextEditor() {
    const height = !!(this.state.isSmallSize && this.state.currentAsideTab) ? this.state.editorHeight : 'auto';
    return (
      <Resizable
        ...
        enable={{
          top: false,
          right: false,
          bottom: !!(this.state.isSmallSize && this.state.currentAsideTab),
          left: true,
          topRight: false,
          bottomRight: false,
          bottomLeft: false,
  ...
  render() {
    return (
      <Row noGutters
           className={classnames('h-100', 'editor--grid', {'aside-opened': this.state.currentAsideTab.length})}>
        <Col xs={12}>
        ...
        {this.state.currentAsideTab && this.renderAside()}
      </Row>
    );
  }
}
